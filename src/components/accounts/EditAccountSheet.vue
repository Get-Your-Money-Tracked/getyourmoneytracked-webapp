...
async function confirmArchive() {
  ...
  // Snapshot for undo before archiving
  const snapshot = { ...props.account }
  try {
      ...
      5000,
      {
        ...
        callback: () => {
          accountsStore.createAccount({
            ...
            startingBalance: snapshot.balance.toString(),
            icon: snapshot.icon,
...
<template>
  ...
  <ResponsiveSheet :open="open" title="Edit Account" test-id="edit-account-sheet" @close="$emit('close')">
